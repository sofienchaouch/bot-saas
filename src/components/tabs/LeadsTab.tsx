...
export const LeadsTab: React.FC = () => {
    ...
    const fetchConvos = async () => {
      try {
        ...
        const res = await fetch(
          `/api/conversations/${selectedTenant.id}?q=${q}&limit=${inboxLimit}`
        );
        if (res.ok) {
  ...
  const filteredLeads = (selectedTenant.leads || []).filter(lead => {
    ...
    const matchesStatus =
      leadsStatusFilter === 'ALL' || lead.status.toUpperCase() === leadsStatusFilter.toUpperCase();
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddLead && (
        <form
          onSubmit={handleManualAddLead}
          className="p-5 border border-white/10 bg-[#080b12] rounded-2xl space-y-4 shadow-2xl relative overflow-hidden"
        >
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1.5">
              <label htmlFor="lead-name" className="text-xs font-semibold text-slate-400 font-mono">
                Customer Name:
              </label>
              <input
                id="lead-name"
          ...
          <div className="space-y-1.5">
            <label htmlFor="lead-note" className="text-xs font-semibold text-slate-400 font-mono">
        ...
        {selectedTenant.autopilotEnabled !== false ? (
          <div className="p-8 text-center bg-[#0d121d]/50 rounded-xl border border-white/5 space-y-3">
            ...
            <p className="text-xs font-semibold text-slate-300">
              Autopilot is Currently Handling Chats
            </p>
            <p className="text-[11px] text-slate-500 max-w-md mx-auto">
              The Gemini AI Bot is autonomously answering customer questions, qualifying leads, and
              syncing bookings. Pause Autopilot to unlock manual chat takeover.
            </p>
          </div>
        ) : (
            ...
            <div className="lg:col-span-4 border border-white/5 rounded-xl bg-[#090d16] flex flex-col max-h-[450px]">
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin">
                {Object.keys(takeoverConvos).length === 0 ? (
                  <p className="text-center text-slate-650 font-mono text-[10px] py-8 select-none">
                    No active threads
                  </p>
                ) : (
                  <>
                    {Object.keys(takeoverConvos).map(key => {
                      ...
                      const matchedLead = selectedTenant.leads?.find(
                        l => l.phone === customerId || l.id === customerId
                      );
                      ...
                      const isMessenger =
                        key.includes('_psid_') || key.toLowerCase().includes('psid');
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center gap-1 mt-1">
                            <span
                              className={`text-[7px] uppercase font-mono px-1 rounded border ${channelColor}`}
                            >
                              {channelLabel}
                            </span>
                            {thread.tags &&
                              thread.tags.map((t: string) => (
                                <span
                                  key={t}
                                  className="text-[7px] font-bold px-1 rounded bg-amber-500/10 border border-amber-500/25 text-amber-400 font-mono"
                                >
                                  {t}
                                </span>
                              ))}
            ...
            <div className="lg:col-span-8 border border-white/5 rounded-xl bg-[#090d16] flex flex-col max-h-[450px]">
              {selectedConvoKey && takeoverConvos[selectedConvoKey] ? (
                          ...
                          <div className="flex items-center gap-1.5 ml-3 font-mono text-[9px]">
                            ...
                            <select
                              ...
                              onChange={async e => {
                                ...
                                try {
                                  const res = await fetch(
                                    `/api/conversations/${selectedTenant.id}/${customerId}/assign`,
                                    {
                                      method: 'POST',
                              ...
                              return (
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const res = await fetch(
                                        `/api/conversations/${selectedTenant.id}/${customerId}/tags`,
                                        {
                                          method: 'POST',
                                          headers: { 'Content-Type': 'application/json' },
                        ...
                        {thread.messages.map((msg: any, index: number) => {
                          ...
                          if (isInternal) {
                            return (
                              <div
                                key={index}
                                className="flex flex-col w-full max-w-[85%] self-center items-center py-1"
                              >
                                <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-300 rounded-xl text-xs font-mono w-full text-center shadow-sm select-none">
                                  <span className="font-bold text-[9px] uppercase tracking-wider text-amber-400 block mb-1">
                                    ⚠️ INTERNAL TEAM NOTE
                                  </span>
                                  {msg.text}
                                </div>
                          ...
                          return (
                            ...
                            >
                                ...
                                {msg.citations && msg.citations.length > 0 && (
                                  <div className="mt-2 pt-1.5 border-t border-white/10 flex flex-wrap gap-1 items-center text-[9px] font-mono opacity-85">
                                    ...
                                    {msg.citations.map((cite: string, idx: number) => (
                                      <span
                                        key={idx}
                                        className="bg-black/20 px-1 py-0.2 rounded border border-white/10"
                                      >
                                        {cite}
                                      </span>
                                    ))}
                              ...
                              <div className="flex items-center gap-1.5 mt-1 text-[8.5px] font-mono text-slate-500">
                                <span>
                                  {isUser ? 'User' : isManual ? '👤 Human Support' : '🤖 AI Agent'}
                                </span>
                                <span>•</span>
                                <span>
                                  {new Date(msg.timestamp).toLocaleTimeString([], {
                      ...
                      >
                          ...
                          <input
                            ...
                            placeholder={
                              isInternalNote
                                ...
                                : 'Type a manual reply response to send...'
                            }
                        ...
                        <div className="flex items-center justify-between font-mono text-[9px] text-slate-400 select-none">
                          <div className="flex items-center gap-1.5">
                            ...
                            />
                            <label
                              htmlFor="takeover-internal-note-check"
                              className="cursor-pointer font-bold hover:text-white transition-colors flex items-center gap-1"
                            >
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            >
                              ...
                              <option value="Hello! Thanks for reaching out. How can I assist you today?">
                                Greet & Welcome
                              </option>
                              <option value="Our business hours are strictly Monday to Friday, from 9:00 AM to 5:00 PM.">
                                Working Hours
                              </option>
                              <option value="I would be happy to book you in! What slot works best for you?">
                                Suggest Booking
                              </option>
                              <option value="Our consultation packages start from $49/month. We accept major credit cards.">
                                Pricing Plans
                              </option>
                            </select>
                          </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-slate-500 font-mono text-xs">
                  ...
                  Select an active channel thread on the sidebar to begin manual takeover text
                  session.
                </div>
              )}
            </div>
                ...
                <td colSpan={5} className="p-12 text-center text-slate-505 font-mono">
                  <div className="max-w-sm mx-auto space-y-2">
                    ...
                    <p className="text-[11px] text-slate-550">
                      Try loosening your search query, selecting "ALL" stages, or triggering leads
                      via the WhatsApp simulator sandbox.
                    </p>
                  </div>
                </td>
              </tr>
                ...
                <tr key={lead.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4 text-slate-400">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[10px] italic max-w-xs truncate text-slate-400">
                        {lead.note || 'AI Agent Captured.'}
                      </p>
                    </div>
                  </td>
                  <td className="p-4 text-center">
                    ...
                    >
                      <option value="New" className="bg-[#0d121d]">
                        NEW
                      </option>
                      <option value="Interested" className="bg-[#0d121d]">
                        INTERESTED
                      </option>
                      <option value="Qualified" className="bg-[#0d121d]">
                        QUALIFIED
                      </option>
                      <option value="Contacted" className="bg-[#0d121d]">
                        CONTACTED
                      </option>
                    </select>
                  </td>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-white text-xs font-bold font-sans">
                Google Sheets Synchronor Completed!
              </h4>
      ...
      {sheetsExportError && (
        <div className="p-4 rounded-xl border border-rose-500/30 bg-rose-500/5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[10.5px] text-rose-300 font-mono mt-0.5">{sheetsExportError}</p>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-white/5 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Configure instant micro-targeted emails dispatched autonomously via connected Gmail
              tokens whenever a customer's status transitions in the CRM list.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-7 space-y-4 text-xs">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="font-mono text-slate-400 block font-semibold">
                  Active Trigger Stage Event:
                </label>
                <select
                ...
                <span className="text-[10px] text-slate-500 font-sans block">
                  Email fires when this specific CRM status is set manually.
                </span>
              </div>

              <div className="space-y-1.5">
                <label className="font-mono text-slate-400 block font-semibold">
                  Associated Workspace Sender:
                </label>
                ...
                <span className="text-[10px] text-slate-500 block">
                  Dispatched on behalf of your connected administrator profile.
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <span className="text-[10px] text-slate-500 block">
                Support tokens:{' '}
                <code className="text-teal-400 font-mono font-bold">{'{customer_name}'}</code>,{' '}
                <code className="text-teal-400 font-mono font-bold">{'{tenant_name}'}</code>
              </span>
            </div>

            <div className="space-y-1.5">
              <label className="font-mono text-slate-400 block font-semibold">
                Auto-Drafted Body Text (Plain / Markdown):
              </label>
              <textarea
                rows={4}
              ...
              <span className="text-[10px] text-slate-500 block">
                Use variables{' '}
                <code className="text-teal-400 font-mono font-bold">{'{customer_name}'}</code> (e.g.
                Marcus), <code className="text-teal-400 font-mono font-bold">{'{status}'}</code>{' '}
                (Qualified), or{' '}
                <code className="text-teal-400 font-mono font-bold">{'{tenant_name}'}</code> inside
                text.
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-2.5">
            ...
            <div className="bg-[#090d16] border border-white/5 rounded-xl p-3.5 font-mono text-[10.5px] text-slate-300 space-y-2 max-h-[290px] overflow-y-auto">
              {nurtureLogs.map((log, idx) => (
                <div
                  key={idx}
                  className="border-b border-white/5 pb-1.5 last:border-b-0 last:pb-0 font-light leading-relaxed"
                >
                  {log}
                </div>
              ))}
      ...
      <div className="bg-[#080b12] border border-white/5 p-6 rounded-2xl space-y-4 shadow-2xl relative overflow-hidden mt-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Test and inspect real-time REST API notifications delivering captured customer objects
            to enterprise pipelines (HubSpot, Salesforce, Zapier webhooks).
          </p>
        </div>
        ...
        {selectedTenant.leads.length === 0 ? (
          <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl text-center text-xs text-slate-500 font-mono">
            No active opportunities found to sync. Visit the{' '}
            <button
              onClick={() => navigate(`/admin/${selectedTenant.id}/simulator`)}
              className="text-blue-400 hover:underline font-bold cursor-pointer"
            >
              WhatsApp Simulator 📱
            </button>{' '}
            thread to harvest client data first!
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
            <div className="lg:col-span-5 space-y-4">
              <div className="space-y-1.5 font-mono text-xs">
                <label htmlFor="webhook-lead-picker" className="font-bold text-slate-400 block">
                  Select Target Contact to Sync:
                </label>
              ...
              {mockPayload && (
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase font-mono block">
                    📦 REST POST payload representation:
                  </span>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase text-slate-405 tracking-wider">
                      Gateway Debug Output Logger
                    </span>
                  </div>
                  <span
                ...
                <div className="space-y-1.5 max-h-56 overflow-y-auto scrollbar-thin text-slate-400">
                  {webhookLogs.length === 0 ? (
                    <p className="text-slate-600 italic select-none">
                      No active webhooks dispatched. Use the trigger above to fire mock CRM
                      requests.
                    </p>
                  ) : (
                    webhookLogs.map((log, idx) => {
                      const isGreen =
                        log.includes('✅') ||
                        log.includes('🎉') ||
                        log.includes('Response Received') ||
