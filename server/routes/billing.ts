...
router.use(
  ["/api/tenant/:id/checkout", "/api/tenant/:id/portal"],
...
router.get("/api/billing/config", asyncHandler(async (req, res) => {
  res.json({
...
router.post("/api/tenant/:id/checkout", asyncHandler(async (req, res) => {
...
router.post("/api/tenant/:id/portal", asyncHandler(async (req, res) => {
  const tenantId = req.params.id;
...
router.post("/api/webhook/stripe", asyncHandler(async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (tenantId && tier) {
        ...
        if (tenant) {
          ...
          tenant.stripeCustomerId = typeof session.customer === "string" ? session.customer : session.customer?.id;
          tenant.stripeSubscriptionId = typeof session.subscription === "string" ? session.subscription : session.subscription?.id;
      ...
      if (tenantId) {
        ...
        if (tenant) {
          ...
          store[tenantId] = tenant;
          await writeTenantsStore(store);
          logger.info({ tenantId, tier: tenant.subscriptionTier, event: event.type }, "[STRIPE WEBHOOK] Subscription updated");
        }
      }
    }
  } catch (err) {
