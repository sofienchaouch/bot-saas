...
const verifyMetaSignature = (
  ...
  try {
    ...
    logger.warn('[META WEBHOOK] Signature verification failed: malformed hex');
...
router.get(
  ...
  (req, res) => {
    ...
    if (mode && token) {
      ...
      const isTokenValid =
        token === SECRET_VERIFY_TOKEN || (tenantId && token === `verify_token_omnibot_${tenantId}`);

      if (mode === 'subscribe' && isTokenValid) {
        logger.info(
          { challenge },
          '[META WEBHOOK] Verification status: APPROVED. Challenge returned'
        );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (!tenant) {
        if (payload.object === 'page') {
          ...
          const foundKey = Object.keys(store).find(k => {
            ...
            return (
              cleanT && cleanFrom && (cleanT.includes(cleanFrom) || cleanFrom.includes(cleanT))
            );
          });
          if (foundKey) {
            tenantId = foundKey;
      ...
      if (tenant && isOverQuota(tenant.subscriptionTier, tenant.messageCount)) {
        logger.warn(
          { tenantId, count: tenant.messageCount, tier: tenant.subscriptionTier },
          '[META WEBHOOK] Tenant is over quota. Rejecting message'
        );
        return res
          .status(403)
          .json({ error: 'Quota Exceeded. Please upgrade your subscription plan.' });
      }
      ...
      if (payload.object === 'page') {
        ...
        if (message) {
          ...
          logger.info(
            { senderName, from, textBody },
            '[META MESSENGER HOOK] Inbound FB Messenger message'
          );
          ...
          if (!tenant) {
            logger.warn(
              { tenantId },
              '[META MESSENGER HOOK] Bypassing because no active tenant matches ID'
            );
            return;
          }
...
Do not wrap your output in markdown codeblocks like \`\`\`json. Return bare clean JSON.`,
          ...
          if (autopilot) {
                    ...
                    responseSchema: {
                      ...
                      properties: {
                        reply: {
                          type: Type.STRING,
                          description:
                            'The direct messaging sentence response to display to the user in the Messenger chat bubble.',
                        },
                        actionTriggered: {
                          type: Type.OBJECT,
                          nullable: true,
                          ...
                          properties: {
                            type: {
                              type: Type.STRING,
                              description:
                                "The action class: 'capture_lead' or 'book_appointment' or 'consult_kb'",
                            },
                            details: {
                              type: Type.STRING,
            ...
            if (actionTriggered) {
              ...
              try {
                ...
                if (actionTriggered.type === 'capture_lead') {
                  ...
                  logger.info(
                    { appointment: newAppt },
                    '[META MESSENGER CALENDAR] Booked Appointment autonomously'
                  );
                } else if (actionTriggered.type === 'purchase_item') {
                  if (!tenant.leads) tenant.leads = [];
                  const matchedLeadIndex = tenant.leads.findIndex(
                    (l: any) =>
                      (actDetails.email &&
                        l.email &&
                        l.email.toLowerCase() === actDetails.email.toLowerCase()) ||
                  ...
                  if (matchedLeadIndex > -1) {
                    ...
                    tenant.leads[matchedLeadIndex].note =
                  ...
                  logger.info(
                    { orderDetails },
                    '[META MESSENGER CRM] E-Commerce Purchase registered autonomously'
                  );
                }
            ...
            // Meter every autopilot-processed inbound message, regardless of
            // whether it triggered a CRM/calendar action.
            {
            ...
            broadcastToTenant(tenantId, {
              ...
              payload: {
                convoKey,
                message:
                  conversations[convoKey].messages[conversations[convoKey].messages.length - 1],
              },
            });
          } else {
            logger.info(
              { tenantId },
              '[META MESSENGER TAKEOVER] Autopilot is disabled for tenant. Session is in manual takeover mode.'
            );
          }
        }
      }
      ...
      if (payload.object === 'whatsapp_business_account') {
        ...
        if (message) {
          ...
          if (!tenant) {
            logger.warn(
              { tenantId },
              '[META WEBHOOK LOG] Bypassing answer hook because no active tenant matches ID'
            );
            return;
          }
          ...
          if (autopilot) {
            ...
            if (currentAi) {
              try {
                ...
                logger.error(
                  { err: aiErr },
                  '[META WEBHOOK AI] Error during AI content generation'
                );
              }
            } else {
              logger.warn(
                '[META WEBHOOK AI] Bypassing Gemini inference because no AI API key is configured.'
              );
            }
            ...
            if (actionTriggered) {
              logger.info(
                ...
                '[META WEBHOOK AI] Action triggered autonomously'
              );
              try {
                const actDetails = JSON.parse(actionTriggered.details || '{}');
                let tenantModified = false;
                ...
                if (actionTriggered.type === 'capture_lead') {
                  ...
                  logger.info(
                    { lead: newLead },
                    '[META WEBHOOK CRM] Captured new Lead autonomously in CRM store'
                  );
                } else if (actionTriggered.type === 'book_appointment') {
                  const newAppt = {
                    id: `appt-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
                  ...
                  logger.info(
                    { appointment: newAppt },
                    '[META WEBHOOK CALENDAR] Booked new Appointment autonomously in CRM calendar'
                  );
                } else if (actionTriggered.type === 'purchase_item') {
                  if (!tenant.leads) tenant.leads = [];
                  const matchedLeadIndex = tenant.leads.findIndex(
                  ...
                  logger.info(
                    { orderDetails },
                    '[META WEBHOOK CRM] E-Commerce Purchase registered autonomously'
                  );
                }
            ...
            recordEvent(tenantId, 'message_sent', 'whatsapp', {
              durationMs,
            ...
            const targetPhoneNumberId =
              tenant.whatsAppVerifiedSid || value?.metadata?.phone_number_id;
            ...
            if (targetPhoneNumberId && accessToken && !isPlaceholderToken(accessToken)) {
              try {
                logger.info(
                  { from, targetPhoneNumberId },
                  '[META WEBHOOK OUTBOUND] Sending real Graph API envelope'
                );
                await sendWhatsAppMessage(targetPhoneNumberId, accessToken, from, botReply);
              } catch (graphErr) {
                logger.error(
                  { err: graphErr },
                  '[META WEBHOOK OUTBOUND] Failed to post message via Meta Graph API'
                );
              }
            } else {
              logger.warn(
                { tenantId },
                '[META WEBHOOK OUTBOUND] Bypassing Graph API delivery because Meta credentials are placeholders or defaults'
              );
            }
          } else {
            logger.info(
              { tenantId },
              '[META WHATSAPP TAKEOVER] Autopilot is disabled for tenant. Session is in manual takeover mode.'
            );
          }
        }
      }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (isOverQuota(tenant.subscriptionTier, tenant.messageCount)) {
      logger.warn(
        { tenantId, count: tenant.messageCount, tier: tenant.subscriptionTier },
        '[TELEGRAM WEBHOOK] Tenant is over quota. Rejecting message'
      );
      return res
        .status(403)
        .json({ error: 'Quota Exceeded. Please upgrade your subscription plan.' });
...
Do not wrap your output in markdown codeblocks like \`\`\`json. Return bare clean JSON.`,
      ...
      try {
        ...
        if (actionTriggered.type === 'capture_lead') {
          ...
          const newAppt = {
            ...
            start:
              actDetails.startStr ||
              new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T10:00:00',
            end:
              actDetails.endStr ||
              new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T10:30:00',
            summary: actDetails.summary || `Autonomous Booking with ${botName}`,
    ...
    if (botToken && !isPlaceholderToken(botToken)) {
      ...
      logger.warn(
        '[TELEGRAM OUTBOUND] Bypassing Telegram API send because token is empty/placeholder.'
      );
    }
...
router.post(
  '/api/webhook/twilio/sms/:tenantId',
  asyncHandler(async (req, res) => {
    const { tenantId } = req.params;
    const store = await readTenantsStore();
    const tenant = store[tenantId];
    ...
    if (isOverQuota(tenant.subscriptionTier, tenant.messageCount)) {
      logger.warn(
        { tenantId, count: tenant.messageCount, tier: tenant.subscriptionTier },
        '[TWILIO SMS] Tenant is over quota. Rejecting message'
      );
      res.type('text/xml');
      return res
        .status(403)
        .send(
          '<Response><Message>Quota Exceeded. Please upgrade your subscription plan.</Message></Response>'
        );
    }
