...
export async function getAnalytics(tenantId: string, days = 30): Promise<AggregatedAnalytics> {
  ...
  const daily: DailyMetrics[] = Array.from(buckets.values())
    ...
    .map(b => ({
      ...b,
      avgResponseTimeMs:
        b.durationSamples > 0 ? Math.round(b.totalDurationMs / b.durationSamples) : 0,
      p95ResponseTimeMs: 0,
    }));
  ...
  const conversionRate =
    totals.messagesReceived > 0 ? Math.round((totals.newLeads / totals.messagesReceived) * 100) : 0;
